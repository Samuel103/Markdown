...
2. Read `context/project-overview.md`, `context/rules.md`, and `context/current-context.md`. Inspect relevant source files when needed to make the plan evidence-based.
3. Update `context/current-context.md` under `# Current feature overview / goals` with a concise, faithful overview of the referenced feature: goals, user-visible behavior, constraints, acceptance criteria, and open questions. Preserve the `# Plans` and `# Notes` sections and all unrelated content.
...
6. Once approved, replace the content under `# Plans` with the agreed plan, preserving the `# Current feature overview / goals` and `# Notes` sections. Confirm that the plan was saved.
7. Create and check out a feature branch. Stop and ask the user if the branch name or Git state requires a decision. Once the branch is ready, wait for a separate `/feature implement` request.
