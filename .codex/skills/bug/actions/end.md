---
name: bug-end
description: Review the current branch's bugfix work, obtain approval for findings, then commit, merge, and push main when clear.
---

# Finalize a bugfix

Use these instructions when the user invokes `/bug end` to close a bugfix in this repository. Review the complete current-branch change set, including committed work not yet in `main` and local worktree changes.
...
3. Review changes for bug correctness, regressions, maintainability, and consistency with the reported bug. Run relevant available validation proportionate to the changes.
...
After approval, apply only the approved corrections, rerun the relevant checks, and repeat the review. If the user declines a non-blocking recommendation, record that decision and continue only when safe. Never treat silence or an earlier request to run `/bug end` as approval to fix a reported finding.
...
The context reset, commit, merge, and normal push to `main` are part of this explicit `/bug end` workflow. Ask again before any action outside this reviewed bugfix workflow.
